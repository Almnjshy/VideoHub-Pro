...
class NetworkClient @Inject constructor() {
     ...
     * Used for calling the yt-dlp resolver service.
     *
     * @param url Full URL to POST to
     * @param jsonBody JSON string to send in the request body
        ...
        return try {
            ...
            client.newCall(requestBuilder.build()).execute().use { response ->
                if (response.isSuccessful) {
                    ...
                    Log.w("NetworkClient", "POST $url failed: HTTP ${response.code}")
                    null
                }
            }
        } catch (e: Exception) {
            Log.w("NetworkClient", "POST $url failed: ${e.message}")
            null
        }
    }
    ...
    fun downloadFile(
        ...
        return try {
            ...
            // HTTP Range header for resuming downloads
            if (resumeFrom > 0 && outputFile.exists() && outputFile.length() == resumeFrom) {
                requestBuilder.header("Range", "bytes=$resumeFrom-")
                Log.i("NetworkClient", "Resuming download from byte $resumeFrom")
            }
