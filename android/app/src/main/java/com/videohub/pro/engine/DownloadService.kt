...
class DownloadService : Service() {
    ...
    @Inject lateinit var notificationHelper: com.videohub.pro.notifications.NotificationHelper
    ...
    private suspend fun updateNotificationSuspend(activeCount: Int) {
        // Get real download progress from the first active task
        val downloading = database.taskDao().getDownloading()
        if (downloading.isNotEmpty()) {
            val task = downloading[0]
            val progress = (task.progress * 100).toInt()
            val speed = task.speedBps
            val eta = task.etaSeconds
            ...
            val notification = notificationHelper.buildDownloadIdleNotification(activeCount)
            val nm = getSystemService(NOTIFICATION_SERVICE) as NotificationManager
            nm.notify(NOTIFICATION_ID, notification)
        }
    }
